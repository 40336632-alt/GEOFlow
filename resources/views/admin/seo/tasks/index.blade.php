...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">SEO发布任务</h1>
        <a href="{{ route('admin.seo.tasks.create') }}"
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('admin.seo.tasks', ['status' => 'completed']) }}"
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">发布链接</th>
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-sm text-gray-500">
                        @if($task->published_url)
                            <a href="{{ $task->published_url }}" target="_blank" class="text-blue-600 hover:text-blue-800">
                                {{ Str::limit($task->published_url, 30) }}
                            </a>
                        @else
                            -
                        @endif
                    ...
                    <td class="px-6 py-4 text-right text-sm">
                        @if($task->isPending() || $task->isFailed())
                            <form action="{{ route('admin.seo.tasks.execute', $task) }}" method="POST" class="inline mr-3">
                                @csrf
                        ...
                        @endif
                        <form action="{{ route('admin.seo.tasks.destroy', $task) }}" method="POST" class="inline"
