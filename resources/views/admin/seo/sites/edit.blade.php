...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <h1 class="text-2xl font-bold text-gray-900">编辑站点</h1>
    </div>

    <form method="POST" action="{{ route('admin.seo.sites.update', $site) }}" class="bg-white rounded-lg shadow">
        @csrf
        @method('PUT')
        ...
        <div class="px-6 py-4 space-y-4">
            <div>
                ...
                <input type="text" name="domain" id="domain" required value="{{ old('domain', $site->domain) }}"
            ...
            <div>
                ...
                <select name="site_type" id="site_type"
                    ...
                    @foreach($siteTypes as $key => $name)
                        <option value="{{ $key }}" {{ old('site_type', $site->site_type) == $key ? 'selected' : '' }}>
                            {{ $name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="remark" id="remark" rows="3"
                          class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
                          placeholder="可选备注信息">{{ old('remark', $site->remark) }}</textarea>
                @error('remark')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
